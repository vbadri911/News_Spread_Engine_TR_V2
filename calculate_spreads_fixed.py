"""
Calculate Spreads: Build credit spreads with OTM options only
Fixed version - only uses out-of-the-money options
"""
import json
import sys
...
def build_credit_spreads():
    """Build credit spreads from OTM options only"""
    print("🔨 Building credit spreads (OTM only)...")
    ...
    for ticker, data in greeks_data.items():
        ...
        otm_calls = [o for o in options if o["type"] == "C" and o["strike"] > stock_price]
        otm_puts = [o for o in options if o["type"] == "P" and o["strike"] < stock_price]
        ...
        # Build BEAR CALL spreads (sell lower OTM call, buy higher OTM call)
        for i, short_call in enumerate(otm_calls[:-1]):
            for long_call in otm_calls[i+1:i+3]:  # Only check next 2 strikes
                width = long_call["strike"] - short_call["strike"]
                
                if 2.5 <= width <= 10:
                    net_credit = short_call["mid"] - long_call["mid"]
                    
                    if net_credit > 0.30:  # Minimum 30 cents
                        max_loss = width - net_credit
                        roi = (net_credit / max_loss * 100) if max_loss > 0 else 0
                        
                        if roi > 10:  # Minimum 10% ROI
                            spread = {
                                "ticker": ticker,
                                "type": "Bear Call",
        ...
        # Build BULL PUT spreads (sell higher OTM put, buy lower OTM put)
        for i, short_put in enumerate(otm_puts[:-1]):
            for long_put in otm_puts[i+1:i+3]:  # Only check next 2 strikes
...
def save_spreads(spreads):
    ...
    for spread in spreads[:3]:
        print(f"   {spread['ticker']} {spread['type']} ${spread['short_strike']}/{spread['long_strike']}: {spread['roi']}% ROI")

def main():
    """Main execution"""
    print("="*60)
    print("STEP 6: Calculate Spreads (FIXED)")
    print("="*60)
